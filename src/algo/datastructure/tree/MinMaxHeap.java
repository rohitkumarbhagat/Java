...
public class MinMaxHeap<T extends Comparable<T>>
{
	...
	public MinMaxHeap(Class<T> claz, int capacity)
	{
		...
		length = 0;
		minHeapArray = (MinMaxHeapNode<T>[]) Array.newInstance(MinMaxHeapNode.class, capacity);
		maxHeapArray = (MinMaxHeapNode<T>[]) Array.newInstance(MinMaxHeapNode.class, capacity);
	}
	...
	public boolean add(T data)
	{
		if (!isFull())
		{
			MinMaxHeapNode<T> newNode = new MinMaxHeapNode<T>(data, length, length);
	...
	{
		boolean result = true;
		if (length <= this.length && length > -1 && index > -1 && index < length)
		{
			// Range correct
			MinMaxHeapNode<T>[] currentheap = heapNo == 1 ? minHeapArray : maxHeapArray;
			while (index > 0)
			{
				int parentIndex = getParent(index, length);
				if (compare(heapNo, currentheap[index], currentheap[parentIndex]) == -1)
				{
	...
	private int compare(int heapNo, MinMaxHeapNode<T> node1, MinMaxHeapNode<T> node2)
	{
		int result = 0;
		if (heapNo == 1)
		{
	...
	private boolean percolate(int heapNo, int index, int length)
	{
		...
		if (index > -1 && index < length)
		{
			...
			if (getLeftChild(index, length) != -1
					&& compare(heapNo, currentHeap[smallest], currentHeap[getLeftChild(index, length)]) == 1)
			{
				// if smallest is > left child
				smallest = getLeftChild(index, length);
			}
			if (getRightChild(index, length) != -1
					&& compare(heapNo, currentHeap[smallest], currentHeap[getRightChild(index, length)]) == 1)
			{
