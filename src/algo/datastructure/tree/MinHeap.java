...
public class MinHeap<T extends Comparable<T>>
{
	...
	public MinHeap(int capacity)
	{
		...
		dataArray = (T[]) Array.newInstance(Integer.class, capacity);
		length = 0;
		this.capacity = capacity;
	...
	public T getKthSmallest(int k)
	{
		...
		if (k > 0 && k <= length)
		{
			// kth smallest element exist
			int count = 0;
			MinHeap<T> tempHeap = new MinHeap<T>(k + 1);
			// 0 element will exist
			tempHeap.add(dataArray[0]);
			while(true){
				kthSmallest = tempHeap.getMin();
				if(++count==k){
					return kthSmallest;
				}
				tempHeap.add(kthSmallest.)
